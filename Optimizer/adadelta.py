        ...
        for layer in layers:
            if hasattr(layer, 'W') and hasattr(layer, 'dW'):
                if linear_idx >= len(self.v_W):
                    ...
                    self.u_W.append(zeros_like(layer.W))
                    self.u_b.append(zeros_like(layer.b))
                ...
                delta_w = layer.dW * sqrt(self.u_W[linear_idx] + self.epsilon) / sqrt(self.v_W[linear_idx] + self.epsilon)
                delta_b = layer.db * sqrt(self.u_b[linear_idx] + self.epsilon) / sqrt(self.v_b[linear_idx] + self.epsilon)
                ...
                if bn_idx >= len(self.v_gamma):
                    ...
                    self.u_gamma.append(zeros_like(layer.gamma))
                    self.u_beta_bn.append(zeros_like(layer.beta))
                ...
                delta_gamma = layer.dgamma * sqrt(self.u_gamma[bn_idx] + self.epsilon) / sqrt(self.v_gamma[bn_idx] + self.epsilon)
                delta_beta = layer.dbeta * sqrt(self.u_beta_bn[bn_idx] + self.epsilon) / sqrt(self.v_beta_bn[bn_idx] + self.epsilon)
