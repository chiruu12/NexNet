...
class Conv2D:
    ...
    def _initialize_weights(self, method):
        ...
        if method == 'he':
            ...
            scale = np.sqrt(2.0 / (self.in_channels * kh * kw + self.out_channels * kh * kw))
        else:
            scale = 0.01
            
        self.W = np.random.randn(self.out_channels, self.in_channels, kh, kw) * scale
    ...
    def _im2col(self, x, kh, kw, stride):
        ...
        col = np.zeros((batch_size, channels, kh, kw, out_h, out_w))
    ...
    def forward(self, x):
        ...
        output = np.dot(self.col, W_col.T) + self.b.T
    ...
    def backward(self, gradient_output):
        ...
        self.dW = np.dot(gradient_output_reshaped.T, self.col).reshape(self.W.shape)
        self.db = np.sum(gradient_output_reshaped, axis=0).reshape(self.b.shape)
        
        dcol = np.dot(gradient_output_reshaped, W_col)
